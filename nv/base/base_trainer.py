...
class BaseTrainer:
    ...
    def __init__(self, model: BaseModel, criterion, optimizer_disc, optimizer_gen, config, device):
        self.device = device
        self.config = config
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        # load discriminator optimizer state from checkpoint only when optimizer type is not changed.
        if (
            ...
            self.logger.warning(
                "Warning: Discriminator optimizer or lr_scheduler given in config file is different "
        ...
        if (
                ...
                checkpoint["config"]["lr_scheduler_gen"] != self.config["lr_scheduler_gen"]
        ):
            self.logger.warning(
                "Warning: Generator optimizer or lr_scheduler given in config file is different "
                "from that of checkpoint. Optimizer parameters not being resumed."
            )
        else:
